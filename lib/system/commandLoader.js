...
async function seeCommands(dir = commandsFolder) {
  ...
  for (const fileOrFolder of items) {
    ...
    try {
      ...
      console.log(chalk.green(`🍃 Técnica cargada: ${chalk.bold(pluginName)}`));
    } catch (e) {
      console.error(chalk.red(`❌ Error cargando técnica ${fileOrFolder}:`), e);
    }
  }
}
...
global.reload = async (_ev, fullPath) => {
  ...
  debounceMap.set(fullPath, setTimeout(async () => {
    ...
    if (!fs.existsSync(fullPath)) {
      console.log(chalk.yellow(`🍃 Técnica eliminada: ${filename}`));
    ...
    try {
      ...
      console.log(chalk.green(`🍃 Técnica recargada: ${filename}`));
      console.log(chalk.cyan(`💪 "El trabajo duro vence al talento"`));
    } catch (e) {
...
function startWatcher() {
  ...
  console.log(chalk.green(`🍃 Observador de técnicas activado`));
  console.log(chalk.cyan(`💚 ¡La juventud explota!`));
}
startWatcher();
