// subs.js - Subbots estilo Rock Lee 🍃
...
const sock = makeWASocket({
  ...
  browser: Browsers.macOS('Rock Lee SubBot 🍃'),
  auth: state,
  markOnlineOnConnect: false, // 🔥 Evita "Esperando mensaje"
  ...
  options: {
    noPresence: true // 🔥 Desactiva presencia
  }
  })

  // 🔥 Ignorar actualizaciones de presencia
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect, isNewLogin, qr }) => {
    ...
    if (connection === 'open') {
      ...
      console.log(chalk.green(`🍃 SUB-BOT conectado: ${chalk.yellow(sock.userId)} 💚`))
    }
    ...
    if (connection === 'close') {
      ...
      if ([401, 403].includes(reason)) {
        if (intentos < 5) {
          console.log(chalk.yellow(`🍃 SUB-BOT ${botId} reconectando (${intentos}/5)...`))
          setTimeout(() => {
            startSubBot(m, client, caption, isCode, phone, chatId, {}, isCommand)
          }, 3000)
        } else {
          console.log(chalk.red(`🍃 SUB-BOT ${botId} falló. Eliminando sesión.`))
          try {
      ...
      if ([DisconnectReason.connectionClosed, DisconnectReason.connectionLost, DisconnectReason.timedOut, DisconnectReason.connectionReplaced].includes(reason)) {
        console.log(chalk.yellow(`🍃 SUB-BOT ${botId} desconectado, reconectando...`))
        setTimeout(() => {
          startSubBot(m, client, caption, isCode, phone, chatId, {}, isCommand)
        }, 3000)
        return
    ...
    const msgCode = await m.reply(`🔑 *Código de emparejamiento:*\n\`\`\`${codeGen}\`\`\`\n\n🍃 *"La juventud explota!"* 💚`);
    ...
    try {
    const msgQR = await client.sendMessage(m.chat, { image: await qrcode.toBuffer(qr, { scale: 8 }), caption: `🍃 *ESCANEA ESTE QR* 🍃\n\n${caption || "Conecta tu sub-bot"}\n\n💚 *"El trabajo duro vence al talento!"*` }, { quoted: m})
    delete commandFlags[senderId]
    setTimeout(async () => {
    try {
...
await client.newsletterFollow(value).catch(err => console.log(chalk.gray(`🍃 Error al seguir canal ${value}`)))
}}}
