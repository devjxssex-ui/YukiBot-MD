// message.js - Procesamiento de mensajes estilo Rock Lee 🍃
...
const DEFAULT_BANNER = 'https://files.catbox.moe/ikre9z.jpg'
const DEFAULT_MENU_ICON = 'https://files.catbox.moe/twzkhn.jpg'
...
// 🍃 Función para mostrar logs de comandos con estilo Rock Lee
function showCommandLog(m) {
  try {
    if (!m || !m.body) return
    ...
    if (hasPrefix && m.body?.length > 0) {
      const chatId = m.chat || "Desconocido"
      const senderId = m.sender || "Desconocido"
      const isGroup = chatId?.endsWith('@g.us') || false
      const senderName = m.pushName || "Usuario"

      const cleanChatId = chatId?.replace("@s.whatsapp.net", "").replace("@g.us", "")
      const cleanSenderId = senderId?.replace("@s.whatsapp.net", "").replace("@g.us", "")
      ...
      console.log(chalk.white(`📝 Comando: ${chalk.green(m.body.slice(0, 50))}`))
      console.log(chalk.white(`👤 Usuario: ${chalk.blue(senderName)}`))
      console.log(chalk.white(`🆔 User ID: ${chalk.yellow(cleanSenderId)}`))
      console.log(chalk.white(`💬 Chat ID: ${chalk.magenta(cleanChatId)}`))
...
export async function smsg(client, m, store) {
  ...
  client.sendButton = async (jid, text = '',footer = '', buffer, buttons, copy, urls, quoted, options) => {
    ...
    const icon = botSettings.icon || DEFAULT_ICON
    const banner = botSettings.banner || DEFAULT_BANNER
  ...
  const namebugsito = botSetting.botname || 'Rock Lee Bot 🍃'
  const namebugg2 = botSetting.namebot || 'Rock Lee'
  const pfp = botSetting.icon || DEFAULT_ICON
  ...
  client.sendContextInfoIndex = async (jid, text = '', options = {}, quoted = null, useQuoted = true, mentionedJid = null, config = {}) => {
    ...
    const banner = config.banner || settings.icon || DEFAULT_ICON
    const botnam = config.title || settings.botname || 'Rock Lee Bot 🍃'
    const namebot2 = config.body || settings.namebot || 'Rock Lee'
    const canalId = settings.newsletter_id || ''
    const canalName = settings.nameid || ''
    const sourceUrl = typeof config.redes === 'string' ? config.redes : typeof settings.link === 'string' ? settings.link : 'https://github.com/devjxssex-ui'
